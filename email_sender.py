...
FROM_NAME  = "TradeIt"
SITE_URL   = "https://tradeit-2nax.onrender.com/"


def _send(to_email: str, subject: str, html: str) -> bool:
    if not resend.api_key:
        print("[email_sender] RESEND_API_KEY не задан")
        return False
    try:
        resend.Emails.send({
...
def send_2fa_code_email(to_email: str, username: str, code: str) -> bool:
    html = f"""<!DOCTYPE html><html><head><meta charset="UTF-8"><style>
        ...
        .body{{padding:36px 40px}}.body p{{color:#4a5568;font-size:15px;line-height:1.6;margin:0 0 16px}}
        ...
        <div class="header"><h1>🔐 TradeIt</h1><p>Двухфакторная аутентификация</p></div>
        <div class="body"><p>Привет, <strong>{username}</strong>!</p><p>Твой код подтверждения:</p>
        <span class="code">{code}</span>
        <p>Код действителен <strong>10 минут</strong>. Никому его не передавай.</p></div>
        <div class="footer">Если ты не запрашивал этот код — просто игнорируй это письмо.</div>
    </div></body></html>"""
    return _send(to_email, "Код подтверждения TradeIt", html)
...
def send_verification_email(to_email: str, username: str, token: str) -> bool:
    ...
    html = f"""<!DOCTYPE html><html><head><meta charset="UTF-8"><style>
        ...
        <div class="header"><h1>🛍️ TradeIt</h1><p>Подтверждение email</p></div>
        <div class="body"><p>Привет, <strong>{username}</strong>!</p>
        ...
        <div class="link">Или перейди по ссылке вручную:<br>{verify_url}</div></div>
        <div class="footer">Ссылка действует 24 часа.<br>Если ты не регистрировался — просто игнорируй это письмо.</div>
    </div></body></html>"""
...
def send_message_notification(to_email: str, sender_name: str, preview: str, link: str) -> bool:
    ...
    html = f"""<!DOCTYPE html><html><head><meta charset="UTF-8"><style>
        ...
        <div class="header"><h1>✉️ TradeIt</h1><p>Новое сообщение</p></div>
        <div class="body"><p>Тебе написал <strong>{sender_name}</strong>:</p>
        <div class="bubble">«{preview}»</div>
        <a class="btn" href="{full_link}">Ответить</a></div>
        <div class="footer">Ты получил это письмо, потому что тебе пришло сообщение на TradeIt.</div>
    </div></body></html>"""
...
def send_purchase_notification(to_email: str, buyer_name: str, ad_title: str, ad_id: int) -> bool:
    ...
    html = f"""<!DOCTYPE html><html><head><meta charset="UTF-8"><style>
        ...
        <div class="header"><h1>🎉 TradeIt</h1><p>Твоё объявление куплено!</p></div>
        <div class="body"><p>Привет! <strong>{buyer_name}</strong> купил твоё объявление:</p>
        <div class="highlight">«{ad_title}»</div>
        <p>Свяжись с покупателем через мессенджер TradeIt, чтобы договориться о передаче товара.</p>
        <a class="btn" href="{full_link}">Посмотреть объявление</a></div>
        <div class="footer">TradeIt — маркетплейс для продажи товаров.</div>
    </div></body></html>"""
...
def send_call_notification(to_email: str, caller_name: str, caller_num: str, call_link: str) -> bool:
    ...
    html = f"""<!DOCTYPE html><html><head><meta charset="UTF-8"><style>
        ...
        <div class="header"><h1>📞 TradeIt</h1><p>Входящий звонок</p></div>
        <div class="body"><p>Тебе звонил <strong>{caller_name}</strong>:</p>
        <div class="bubble">Номер: {caller_num}</div>
        <p>Если ты пропустил звонок — нажми кнопку ниже, чтобы перезвонить.</p>
        <a class="btn" href="{full_link}">📞 Перезвонить</a></div>
        <div class="footer">TradeIt — маркетплейс для продажи товаров.</div>
    </div></body></html>"""
