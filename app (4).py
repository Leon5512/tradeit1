...
import os, time, functools, uuid as uuid_lib
...
def register():
    if request.method == "POST":
        ...
        if db.execute("SELECT id FROM users WHERE username=? OR email=?", (username, email)).fetchone():
            flash("Пользователь уже существует", "error"); return redirect(url_for("register"))
        count = db.execute("SELECT COUNT(*) FROM users").fetchone()[0]
        is_admin = 1 if count == 0 else 0
        db.execute("INSERT INTO users (username,email,password,is_admin,created_at) VALUES (?,?,?,?,?)",
                   (username, email, generate_password_hash(password), is_admin, int(time.time())))
        db.commit()
...
def login():
    if request.method == "POST":
        ...
        if user and check_password_hash(user["password"], password):
            if user["is_banned"]: flash("Аккаунт заблокирован.", "error"); return redirect(url_for("login"))
            session.clear()
            session["user_id"] = user["id"]
            session["username"] = user["username"]
            session["is_admin"] = bool(user["is_admin"])
            session["admin_unlocked"] = False
            flash("Добро пожаловать!", "success"); return redirect(url_for("index"))
...
def api_support_send():
    ...
    try:
        ...
        ai_text = "Извините, сейчас не могу ответить. Попробуйте позже."
...
def msn_chat(other_profile_id):
    ...
    return render_template("msn_chat.html", me=me, other=other, msgs=msgs)
...
def msn_group_chat(group_id):
    ...
    return render_template("msn_group.html", me=me, group=group, msgs=msgs, members=members)
...
def api_msn_call_initiate(callee_profile_id):
    """Звонящий инициирует звонок — создаём запись и возвращаем call_id"""
    db = get_db()
    me = msn_profile(session["user_id"])
    ...
    db.execute(
        "INSERT INTO msn_calls (call_id, caller_id, callee_id, status, created_at) VALUES (?,?,?,?,?)",
...
def api_msn_call_signal():
    """Отправить SDP offer/answer или ICE candidate"""
    data = request.get_json()
    call_id = data.get("call_id")
    sig_type = data.get("type")     # "offer" | "answer" | "ice" | "reject" | "hangup"
    ...
    me = msn_profile(session["user_id"])
    db.execute(
        "INSERT INTO msn_call_signals (call_id, from_id, to_id, type, payload, created_at) VALUES (?,?,?,?,?,?)",
        (call_id, me["id"], to_id, sig_type, payload, int(time.time()))
    )
    if sig_type in ("reject", "hangup"):
        db.execute("UPDATE msn_calls SET status=?, ended_at=? WHERE call_id=?",
                   (sig_type, int(time.time()), call_id))
    elif sig_type == "answer":
        db.execute("UPDATE msn_calls SET status='active', started_at=? WHERE call_id=?",
...
def api_msn_call_poll(call_id):
    """Получить новые сигналы для этого звонка (polling)"""
    since = int(request.args.get("since", 0))
    db = get_db()
    me = msn_profile(session["user_id"])
    rows = db.execute(
        "SELECT * FROM msn_call_signals WHERE call_id=? AND to_id=? AND id>? ORDER BY id",
        (call_id, me["id"], since)
    ).fetchall()
...
def api_msn_call_incoming():
    ...
    call = db.execute(
        """SELECT c.*, p.msn_username as caller_name, p.msn_number as caller_num, p.avatar as caller_av
